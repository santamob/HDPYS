...
class Gradation_model extends CI_Model {
    ...
    public function get_pool_levels($year,$pool_id,$period)
    {
        $this->db->select('havuz_performans_duzeyleri.id AS id,level,percantage,color');    
        $this->db->from('havuz_performans_duzeyleri');
        $this->db->join('dagilim_anahtarlari', 'havuz_performans_duzeyleri.id_da = dagilim_anahtarlari.id','INNER');
        $this->db->join('pools_has_keys', 'pools_has_keys.id_da = dagilim_anahtarlari.id','INNER');
        $this->db->where("idhavuz", $pool_id);
        $this->db->where("phk_status", 1);
        $this->db->where("pools_has_keys.year", $year);
        $this->db->where("havuz_performans_duzeyleri.status", 1);
    ...
    public function get_pool_levels_with_join($for_merge="",$year="",$pool_id="",$period="",$person_count = "",$orderable=""){

        // SELECT `havuz_performans_duzeyleri`.`id` AS `id`, `count_person`, `level`, `value`, `percantage`, `havuz_performans_duzeyleri`.`year`, `period`, `merge_flag` 
        // FROM `havuz_performans_duzeyleri` 
        // INNER JOIN `test_kademe_table` ON `havuz_performans_duzeyleri`.`id` = `test_kademe_table`.`hpd_id` 
        // INNER JOIN `dagilim_anahtarlari` ON `havuz_performans_duzeyleri`.`id_da` = `dagilim_anahtarlari`.`id` 
        // INNER JOIN `pools_has_keys` ON `pools_has_keys`.`id_da` = `dagilim_anahtarlari`.`id` 
        // WHERE `havuz_performans_duzeyleri`.`year` = '2022' 
        // AND `period` = '3' AND `count_person` = '7' AND `idhavuz` = '71' 
        // AND `havuz_performans_duzeyleri`.`status` = 1 
        // AND `phk_status` = 1 
        // ORDER BY `havuz_performans_duzeyleri`.`id`
 
        $this->db->select('havuz_performans_duzeyleri.id AS id,count_person,level,value,percantage,havuz_performans_duzeyleri.year,period,merge_flag,color');    
        $this->db->from('havuz_performans_duzeyleri');
        $this->db->join('test_kademe_table', 'havuz_performans_duzeyleri.id = test_kademe_table.hpd_id','INNER');
        $this->db->join('dagilim_anahtarlari', 'havuz_performans_duzeyleri.id_da = dagilim_anahtarlari.id','INNER');
        $this->db->join('pools_has_keys', 'pools_has_keys.id_da = dagilim_anahtarlari.id','INNER');
        if(!empty($year)){
            $this->db->where("havuz_performans_duzeyleri.year", $year);
        }
        if(!empty($period)){
            $this->db->where("period", $period);
        }
        ...
        if(!empty($for_merge) && $for_merge){
            $this->db->group_by("merge_flag"); //ben bunu sildim
        }
    ...
    public function anahtarNameisExist($name)
    {
        $this->db->where("name", $name); 
        $query = $this->db->get("dagilim_anahtarlari");
        //return $query->result();
        return  ($query->num_rows() > 0 ) ? TRUE : FALSE;
    }


    public function duzeylerForOrganizasyon($year,$period)
    {

        // SELECT 
        // dagilim_anahtarlari.id,
        // dagilim_anahtarlari.name 
        // FROM `dagilim_anahtarlari`
        // INNER JOIN `havuz_performans_duzeyleri` ON dagilim_anahtarlari.id = havuz_performans_duzeyleri.id_da
        // WHERE YEAR = 20
        //23 AND PERIOD = 3
        // GROUP BY NAME 
        $this->db->select("dagilim_anahtarlari.id,dagilim_anahtarlari.name");
        $this->db->from('dagilim_anahtarlari');
        $this->db->join('havuz_performans_duzeyleri', 'dagilim_anahtarlari.id = havuz_performans_duzeyleri.id_da','INNER');
        $this->db->where("year", $year);
        $this->db->where("period", $period); 
    ...
    public function createLevelsWithYearAndPeriod($data)
    {
            ...
            $insert_key = [
                "name"=>$data["kademelendirme_ismi"],
                "is_over_ten"=>($data["isForTen"] == false) ? 1:0,
                "year"=>$data["yil"]
            ];
            $this->db->insert("dagilim_anahtarlari", $insert_key);
            $key_id = $this->db->insert_id();

            $colors = ["danger","primary","warning","info","success"];
